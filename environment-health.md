...
| Emblem-3D family, FUGUEJUKEBOX, MemoryPalace, AlchemyBeatEmUp, promptarchaeology | **Hardcoded absolute `C:\Dev\...` paths** + cross-project reads (e.g. `EmblemRoguelike/assets/fugues.json`, `Claudiens/site/images/emblems` at `AlchemyBeatEmUp/scripts/03_pixelate_emblems.py:27`, `megabase/megabase.db` for promptarchaeology, `WIKI=C:\Dev\wiki`) | Silent break / wrong data if a sibling project moves or is renamed | Treat sibling paths as a dependency; grep for `C:\Dev` literals before moving anything |
...
| Emblem-3D family (HPin3D et al.) | `<canvas>` with `position:fixed;inset:0` but **no explicit width/height** — a replaced element keeps its intrinsic **300×150**, so the renderer draws the whole app into a tiny top-left box | Console clean, accessibility snapshot/`eval` all look fine; only a screenshot reveals it | Set `width:100vw;height:100vh` on the canvas; **verify by screenshot** — see [[concept_opportunity_audit]] |
| HPin3D | Referenced assets (`/images/emblems/*.jpg` in `emblems.json`) **did not exist in-repo**; identical sets sat in sibling projects | Blank/placeholder textures, silent 404s under a WebGL canvas | Copy the web-optimized set in-repo (self-host); use **relative** paths so GitHub Pages project URLs resolve |

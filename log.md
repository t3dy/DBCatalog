...
## [2026-05-23] build | DH Admin Panel (admin/)
- Built `admin/app.py`: Flask 3.x admin panel with five routes — project dashboard, entity browser, entity edit form (GET/POST), and rebuild trigger. Config-driven dynamic SQL (table/column names from config.json only; values fully parameterized). Junction-table and FK relationship patterns both supported.
- Built `admin/config.json`: Project registry with example entries for RenMagDB (figures, terms tables) and HermeticDB (figures, claims tables). Declares editable_fields, tag_fields, review_field, confidence_field, and relationships per table.
- Built `admin/templates/`: base.html (dark admin theme, CSS variables, badge system, btn-group for status selectors, tag chip components), index.html (project dashboard), project.html (entity browser table with review/confidence badges), entity.html (full edit form: prose textareas, tag chips with JS sync, button-group status selectors, relationship checkboxes, rebuild trigger).

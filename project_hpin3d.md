...
## Fragile parts
`STATUS.md` / `TECH_STACK.md` are stale (claim "NOT STARTED" / list uninstalled Vite/Theatre/Zustand; the real app is CDN-only) — stale-docs trap. Manual cache-bust (`main.js?v=12`). Canvas-sizing + missing-asset traps now fixed but recorded in [[environment-health]]. Cross-project DB coupling to sibling HP/Atalanta DBs.

## Status & next
Shipped and live. Reconcile stale planning docs to reality. Possible next: extend tours to draw on the broader corpus ([[project_claudiens]], [[project_hypnerotomachia]] marginalia); deeper HP rooms.
